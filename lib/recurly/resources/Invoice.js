 ...
 * @prop {number} due - The outstanding balance remaining on this invoice.
 * @prop {datetime} dueAt - Date invoice is due. This is the date the net terms are reached.
 * @prop {string} id - Invoice ID
 * @prop {InvoiceLineItems} lineItems - Line items are grouped by the role they play.
 ...
 * @prop {string} subscriptionId - If the invoice is charging or refunding for a subscription, this is its ID.
 ...
 * @prop {string} type - The original invoice will have a type of `purchase`. Any refunds or voids will create a negative invoice to cancel out the original. `line_item_refund` indicates that specific line items were refunded, while `open_amount_refund` only indicates money was refunded.
